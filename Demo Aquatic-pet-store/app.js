    ...
    return `
      <button type="button" class="categoryCard" data-category-card="${escapeHtml(category.id)}">
        <span class="categoryImageWrap">
          ${featured ? `<img src="${escapeHtml(imageSrc(featured.imageUrl, 700))}" alt="${escapeHtml(category.name)}" onerror="this.onerror=null;this.src='${FALLBACK_IMAGE}'">` : `<span class="categoryFallback">${categoryEmoji(category.name)}</span>`}
        </span>
        <span class="categoryCardCopy"><b>${categoryEmoji(category.name)} ${escapeHtml(category.name)}</b><small>${categoryProducts.length} available listing${categoryProducts.length === 1 ? "" : "s"}</small></span>
...
function renderCategoryChips() {
  ...
  const chips = [{id: "", name: "All Products"}, ...categories];

  chipRoot.innerHTML = chips.map(category => `
    <button
      type="button"
      class="categoryChip ${selected === category.id ? "active" : ""}"
    ...
    return `
      <article class="card premiumProductCard">
        <div class="productMedia">
          ...
          <span class="productCategoryBadge">${categoryEmoji(p.categoryName)} ${escapeHtml(p.categoryName || "Aqua Pet")}</span>
        ...
        <div class="productCardBody">
          ...
          <div class="productPriceRow">
            <div><small>PRICE</small><b>${peso(p.price)}</b></div>
            <span>${soldOut ? "Unavailable" : `${stock} available`}</span>
          </div>
...
function buildLocalDemoStore() {
  ...
  return {
    settings: {
      ...
      paymentNote: "Demo preview only. Connect Apps Script to enable live orders and payments.",
      logoUrl: "",
...
$("checkoutForm").onsubmit = async event => {
  event.preventDefault();
  if (LOCAL_DEMO_MODE) {
    $("status").textContent = "Demo preview only. Connect config.js to your Apps Script Web App before submitting live orders.";
    return;
  }
...
$("reviewForm").onsubmit = async event => {
  event.preventDefault();
  if (LOCAL_DEMO_MODE) {
    $("reviewStatus").textContent = "Demo preview only. Connect Apps Script before posting a live review.";
    return;
  }
