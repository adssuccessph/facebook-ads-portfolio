...
function setupStore() {
  ...
  if (categoriesSheet.getLastRow() === 1) {
    categoriesSheet.appendRow([uid("cat"), "Betta Fish", new Date()]);
    categoriesSheet.appendRow([uid("cat"), "Guppy", new Date()]);
  }
...
function createOrder(r) {
  ...
  try {
    ...
    appendByHeaders(sh("Orders"), {
      ...
      trackingNumber: "",
      trackingUrl: "",
      statusHistoryJson: JSON.stringify([{status: "Pending", at: new Date().toISOString()}]),
      shippedAt: "",
      completedAt: "",
      cancelledAt: "",
      shipmentEmailSentAt: "",
